...
NRS Workbench brings your local runners and Git working trees into one desktop app. In the downloadable **v0.18.0 Public Preview**, check runner status and current jobs, inspect locally retained job history, see your PC's CPU, memory and disk usage, search and sort runners, review local repositories, switch between existing branches, and use guarded Fetch, Pull, Push and Commit actions. The interface is available in English and Spanish.

**Development status:** The optional automatic runner queue is implemented on `main` after the v0.18.0 release. It is **not included in the downloadable v0.18.0 ZIP**. It can keep one or two runners connected and rotate idle runners while GitHub holds jobs for stopped runners. See [Automatic runner queue](#automatic-runner-queue) for its limitations.
...
**En español:** La descarga v0.18.0 permite gestionar runners y repositorios Git locales, consultar trabajos y recursos del PC, y buscar y ordenar runners. La cola automática está integrada en `main`, pero **no está incluida en el ZIP v0.18.0**. La interfaz está disponible en español e inglés.
...
> **Available on development `main` only.** This feature is not present in the published v0.18.0 Windows ZIP. No newer version has been released yet.
