# Public Readiness Audit — v0.14.3

Date: 2026-09-09

This audit was performed before the first intended public preview. v0.14.3 also corrects the audit itself so generated `bin/` and `obj/` output created by the readiness build is excluded from source-path scanning on Windows. It combines static source review with isolated Git-behavior tests. The audit environment did not provide the .NET SDK, so the final Windows/.NET compilation remains a required CI/local check.
...
```powershell
dotnet build .\RunnerManager.sln -c Release
dotnet run --project .\tests\RunnerManager.SmokeTests\RunnerManager.SmokeTests.csproj -c Release
